...
int get_shm_recog_result(recog_result **rr, int init) {
    ...
    if (init) {
        ...
        getflags = 0;
        size     = 0;
        // atflags = SHM_RDONLY; // the kernal can restrict to access of shared
        // memory by using SHM_RDONLY.
        atflags = SHM_RND;
    }
    ...
    if ((*rr = (recog_result *)shmat(id, NULL, atflags)) ==
        (recog_result *)-1) {
    ...
    // all zero values. Reference:
    // https://pubs.opengroup.org/onlinepubs/009695399/functions/shmget.html
