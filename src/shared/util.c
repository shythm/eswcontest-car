...
void maintain_playback_rate(struct rate_control *p) {
    ...
    usecs_to_sleep =
        usecs_between_frames - usecs_since_last_frame + p->usecs_to_sleep;

    if (usecs_to_sleep < 0) usecs_to_sleep = 0;

    /* mark() has a limitation that >1s time deltas will make the whole
     * loop diverge. Workaround that limitation by clamping our desired sleep
     * time to a maximum. TODO: Remove when mark() is in better shape. */
    if (usecs_to_sleep >= 1000000) usecs_to_sleep = 999999;

    /* We filter a bit our rate adaptation, to avoid being too "choppy".
     * Adjust the "alpha" value as needed. */
    p->usecs_to_sleep =
        ((67 * p->usecs_to_sleep) + (33 * usecs_to_sleep)) / 100;
