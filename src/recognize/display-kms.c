...
static struct omap_bo *alloc_bo(struct display *disp, uint32_t bpp,
    ...
    if (bo) {
        ...
        *pitch     = width * bpp / 8;
        if (bo_flags & OMAP_BO_TILED) *pitch = ALIGN2(*pitch, PAGE_SHIFT);
    }

    return bo;
}

struct buffer *alloc_buffer(struct display *disp, uint32_t fourcc, uint32_t w,
                            uint32_t h, bool addfb) {
    ...
    if (!fourcc) fourcc = FOURCC('A', 'R', '2', '4');

    if (fourcc == FOURCC('R', 'G', 'B', '3') ||
        fourcc == FOURCC('B', 'G', 'R', '3'))
    ...
    case FOURCC('B', 'G', '2', '4'):
        buf->nbo   = 1;
        buf->bo[0] = alloc_bo(disp, 24, buf->width, buf->height, &bo_handles[0],
                              &buf->pitches[0]);
        break;
    ...
    case FOURCC('R', 'A', '2', '4'):
        buf->nbo   = 1;
        buf->bo[0] = alloc_bo(disp, 32, buf->width, buf->height, &bo_handles[0],
                              &buf->pitches[0]);
        break;
    case FOURCC('U', 'Y', 'V', 'Y'):
    case FOURCC('Y', 'U', 'Y', 'V'):
        buf->nbo   = 1;
        buf->bo[0] = alloc_bo(disp, 16, buf->width, buf->height, &bo_handles[0],
                              &buf->pitches[0]);
        break;
    case FOURCC('N', 'V', '1', '2'):
        if (disp->multiplanar) {
    ...
    case FOURCC('I', '4', '2', '0'):
        buf->nbo   = 3;
        buf->bo[0] = alloc_bo(disp, 8, buf->width, buf->height, &bo_handles[0],
                              &buf->pitches[0]);
    ...
    if (addfb) {
        ret =
            drmModeAddFB2(disp->fd, buf->width, buf->height, fourcc, bo_handles,
                          buf->pitches, offsets, &buf_kms->fb_id, 0);
        if (ret) {
...
static struct buffer **alloc_buffers(struct display *disp, uint32_t n,
...
static struct buffer **get_buffers(struct display *disp, uint32_t n) {
    return alloc_buffers(disp, n, 0, disp->width, disp->height);
}

static struct buffer **get_vid_buffers(struct display *disp, uint32_t n,
                                       uint32_t fourcc, uint32_t w,
                                       uint32_t h) {
...
static int post_buffer(struct display *disp, struct buffer *buf) {
    struct display_kms *disp_kms = to_display_kms(disp);
    ...
    for (i = 0; i < disp_kms->connectors_count; i++) {
        ...
        if (!disp_kms->current) {
             ...
             */
            MSG("Setting mode %s on connector %d, crtc %d", connector->mode_str,
                connector->id, connector->crtc);

            ret = drmModeSetCrtc(disp->fd, connector->crtc, buf_kms->fb_id, x,
                                 0, &connector->id, 1, connector->mode);
        ...
        if (ret) {
            ERROR("Could not post buffer on crtc %d: %s (%d)", connector->crtc,
                  strerror(errno), ret);
            last_err = ret;
...
static int post_vid_buffer(struct display *disp, struct buffer *buf, uint32_t x,
    ...
    for (i = 0; i < disp_kms->connectors_count; i++) {
        ...
        if (!disp_kms->ovr[i]) {
            ...
            for (j = 0; j < disp_kms->plane_resources->count_planes; j++) {
                if (used_planes & (1 << j)) { continue; }
                drmModePlane *ovr = drmModeGetPlane(
                    disp->fd, disp_kms->plane_resources->planes[j]);
        ...
        if (buf->noScale) {
            ...
            ret = drmModeSetPlane(
                disp->fd, disp_kms->ovr[i]->plane_id, connector->crtc,
                buf_kms->fb_id, 0,
        ...
        if (ret) {
            ERROR("failed to enable plane %d: %s", disp_kms->ovr[i]->plane_id,
                  strerror(errno));
        }
    }
...
static void close_kms(struct display *disp) {
    struct display_kms *disp_kms = to_display_kms(disp);

    omap_device_del(disp->dev);
    disp->dev = NULL;
    if (used_planes) { used_planes >>= 1; }
    if (--ndisplays == 0) { close(global_fd); }

    free(disp_kms);
    disp = NULL;
}

static void connector_find_mode(struct display *disp, struct connector *c) {
    struct display_kms *disp_kms = to_display_kms(disp);
    ...
    for (i = 0; i < disp_kms->resources->count_connectors; i++) {
        connector =
            drmModeGetConnector(disp->fd, disp_kms->resources->connectors[i]);
        ...
        for (j = 0; j < connector->count_modes; j++) {
            ...
            if (!strcmp(c->mode->name, c->mode_str)) break;
        }
    ...
    for (i = 0; i < disp_kms->resources->count_encoders; i++) {
        c->encoder =
            drmModeGetEncoder(disp->fd, disp_kms->resources->encoders[i]);
        ...
        if (c->encoder->encoder_id == connector->encoder_id) break;
...
struct display *disp_kms_open(int argc, char **argv) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (!strcmp("-1", argv[i])) {
            ...
            connector->crtc = -1;
            argv[i++]       = NULL;
            if (sscanf(argv[i], "%d:%64s", &connector->id,
                       connector->mode_str) != 2 &&
                sscanf(argv[i], "%d@%d:%64s", &connector->id, &connector->crtc,
                       connector->mode_str) != 3) {
                // TODO: we could support connector specified as a name too, I
                // suppose
                ERROR("invalid arg: %s", argv[i]);
                goto fail;
            }
...
void disp_usage(void) {
    ...
    MSG("\t--no-post\tDo not post buffers (disables screen updates) for "
        "benchmarking. Rate can still be controlled.");
...
static int empty_post_buffer(struct display *disp, struct buffer *buf) {
    return 0;
}

static int empty_post_vid_buffer(struct display *disp, struct buffer *buf,
...
int disp_post_buffer(struct display *disp, struct buffer *buf) {
    int ret;

    ret = disp->post_buffer(disp, buf);
    if (!ret) maintain_playback_rate(&disp->rtctl);
    return ret;
}

/* flip to / post the specified video buffer */
int disp_post_vid_buffer(struct display *disp, struct buffer *buf, uint32_t x,
                         uint32_t y, uint32_t w, uint32_t h) {
    int ret;
...
static void fillRGB4(char *virtual, int n, int width, int height, int stride) {
    int i, j;
...
static void fill420(unsigned char *y, unsigned char *u, unsigned char *v,
                    int cs /*chroma pixel stride */, int n, int width,
    ...
    for (j = 0; j < height; j += 2) {
        ...
        for (i = 0; i < width; i += 2) {
            div_t    d = div(n + i + j, width);
            uint32_t rgb =
                0x00130502 * (d.quot >> 6) + 0x000a1120 * (d.rem >> 6);
            unsigned char *rgbp = (unsigned char *)&rgb;
            unsigned char  y    = (0.299 * rgbp[RED]) + (0.587 * rgbp[GREEN]) +
                              (0.114 * rgbp[BLUE]);
...
static void fill422(unsigned char *virtual, int n, int width, int height,
                    int stride) {
    int i, j;
    /* paint the buffer with colored tiles */
    for (j = 0; j < height; j++) {
...
void fill(struct buffer *buf, int n) {
    int i;
    ...
    case 0: {
        assert(buf->nbo == 1);
        fillRGB4(omap_bo_map(buf->bo[0]), n, buf->width, buf->height,
                 buf->pitches[0]);
        break;
    }
    case FOURCC('Y', 'U', 'Y', 'V'): {
        assert(buf->nbo == 1);
        fill422(omap_bo_map(buf->bo[0]), n, buf->width, buf->height,
                buf->pitches[0]);
        break;
    }
    case FOURCC('N', 'V', '1', '2'): {
...
void set_z_order(struct display *disp, uint32_t plane_id) {
    ...
    if (plane_id == disp->overlay_p.id) val = disp->overlay_p.z_val;
    else
        val = 0;

    DBG("z-order plane_id:%d, val:%d\n", plane_id, val);

    i = drmModeObjectSetProperty(disp->fd, plane_id, DRM_MODE_OBJECT_PLANE, 7,
                                 val);
    if (i < 0) { ERROR("set z-order for plane id %d failed!!\n", plane_id); }
}
...
void set_global_alpha(struct display *disp, uint32_t plane_id) {
    ...
    if (plane_id == disp->overlay_p.id) val = disp->overlay_p.glo_alp;
    else
        val = 127; // - semi transparent

    DBG("set_global_alpha plane_id:%d, val:%d\n", plane_id, val);

    i = drmModeObjectSetProperty(disp->fd, plane_id, DRM_MODE_OBJECT_PLANE, 8,
                                 val);
    if (i < 0) { ERROR("set global alpha for plane id %d failed\n", plane_id); }
}
...
void set_pre_multiplied_alpha(struct display *disp, uint32_t plane_id) {
    ...
    if (plane_id == disp->overlay_p.id) val = disp->overlay_p.pre_mul_alp;
    else
        val = 0;

    i = drmModeObjectSetProperty(disp->fd, plane_id, DRM_MODE_OBJECT_PLANE, 9,
                                 val);
    if (i < 0) {
...
/**
  ...
  * @retval  none
  */
void alloc_overlay_plane(struct display *disp, uint32_t fourcc, int x, int y,
                         int w, int h) {
...
void update_overlay_disp(struct display *disp) {
    ...
    if (disp->overlay_p_bo->noScale) {
        if (drmModeSetPlane(disp->fd, pplane->id, c->crtc, buf_kms->fb_id,
                            flags, pplane->x, pplane->y, pplane->xres,
                            pplane->yres, 0, 0, pplane->xres << 16,
                            pplane->yres << 16)) {
            ERROR("failed to update overlay (noscale)\n");
        }
    } else {
        if (drmModeSetPlane(disp->fd, pplane->id, c->crtc, buf_kms->fb_id,
                            flags, 0, 0, c->mode->hdisplay, c->mode->vdisplay,
                            pplane->x << 16, pplane->y << 16,
                            pplane->xres << 16, pplane->yres << 16)) {
...
int get_framebuf(struct buffer *buf, unsigned char **ppfbuf) {
    ...
    if (buf->nbo == 1) {
        ...
        if (ppfbuf[0] == NULL) ret = -1;
    } else if (buf->nbo == 2) {
        ppfbuf[0] = (unsigned char *)omap_bo_map(buf->bo[0]);
        ppfbuf[1] = (unsigned char *)omap_bo_map(buf->bo[1]);
        if (ppfbuf[0] == NULL || ppfbuf[1] == NULL) ret = -1;
    } else {
