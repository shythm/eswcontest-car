...
void _getPerspectiveTransform(Mat *M, Point2f *src) {
  ...
  // I don't know why but getPerspeciveTransform does not work properly when all
  // Point2f positions are integer.
...
bool inRange(Point x) { return x.x >= 0 && x.y >= 0 && x.x < W && x.y < H; }
...
void detect_lane(recog_arg *arg, vector_lane *result) {
  ...
  adaptiveThreshold(img, img, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY,
                    31, -20);
