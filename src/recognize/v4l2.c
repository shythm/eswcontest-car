...
struct v4l2 *v4l2_open(uint32_t fourcc, uint32_t width, uint32_t height) {
    ...
    if ((format.fmt.pix.width == 0) || (format.fmt.pix.height == 0) ||
        ...
        ERROR("invalid capture settings '%dx%d@%4s'", format.fmt.pix.width,
              format.fmt.pix.height, (char *)&format.fmt.pix.pixelformat);
        goto fail;
    }
...
int v4l2_reqbufs(struct v4l2 *v4l2, uint32_t n) {
    ...
    if (ret < 0) ERROR("REQBUFS failed: %s\n", strerror(errno));
    ...
    if ((reqbuf.count != n) || (reqbuf.type != V4L2_BUF_TYPE_VIDEO_CAPTURE) ||
...
int v4l2_streamon(struct v4l2 *v4l2) {
    ...
    if (ret) { ERROR("VIDIOC_STREAMON failed: %s (%d)", strerror(errno), ret); }
...
int v4l2_qbuf(struct v4l2 *v4l2, int dmafd, int index) {
    ...
    //  vipfd, buf.type, V4L2_BUF_TYPE_VIDEO_CAPTURE, buf.memory,
    //  V4L2_MEMORY_DMABUF, buf.index, buf.m.fd);

    ret = ioctl(v4l2->fd, VIDIOC_QBUF, &buf);
    if (ret < 0) ERROR("QBUF failed: %s, index = %d\n", strerror(errno), index);
    ...
    for (i = 0; i < v4l2->nbufs; i++) {
        if (v4l2->v4l2bufs[i].m.fd == fd) { v4l2buf = &v4l2->v4l2bufs[i]; }
    }
    ...
    if (v4l2buf->timestamp.tv_sec)
        DBG("%x: DISP latency = %d.%6d", (unsigned int)tid,
            (int)disp_lat.tv_sec, (int)disp_lat.tv_usec);
    ...
    if (ret) { ERROR("VIDIOC_QBUF failed: %s (%d)", strerror(errno), ret); }
    return ret;
#endif
}
...
int v4l2_dqbuf(struct v4l2 *v4l2, int *field) {
    ...
    if (ret) { ERROR("VIDIOC_DQBUF failed: %s (%d)", strerror(errno), ret); }
    ...
    *field = v4l2buf.field;
    ...
    DBG("%x: DQ latency = %d.%6d", (unsigned int)tid, (int)dq_lat.tv_sec,
        (int)dq_lat.tv_usec);
