...
- 카메라로 영상을 읽어 그 영상에서 간선만을 추출해 화면에 검은 바탕에 흰색 간선을 그린다.
...
### exam_cv.h / exam_cv.cpp

 OpenCV 라이브러리를 이용하여 이미지 파일을 로드하고, BGR을 RGB로 변환하거나, 얼굴인식, 이미지 병합, Canny Edge Detection, Hough Transform 등을 지원한다.
...
#include <sys/types.h>
 ...
 - capture_thread
   - V4L2를 통해 카메라 이미지를 읽고 이를 VPE로 가공한 다음 OpenCV의 Hough Transform을 적용한다. 그 후 이미지를 DumpMsg 타입으로 메세지 큐에 보낸다.
 - capture_dump_thread
...
## 실행 결과
 카메라에 들어오는 이미지에서 간선 형태를 가진 부분을 추출하여 검은 바탕에 흰색 간선을 그린다.